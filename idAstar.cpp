...
//	long tStart = System.currentTimeMillis();
	while(1){
		if(rt!=1){
			...
			rt =  search_id_Astar(puzzle, 0, -1,cutoff);
		}
		else{
			break;
		}
...
int search_id_Astar(Puzzle_struct *puzzle, int cost_reach_node , int action , int cutoff ){
	...
	if(distance_to_final == 0){
		...
		cout << "time = " << ((float)(t-ttime))/CLOCKS_PER_SEC << " seconds." << endl;
		exit(0);
	}
	...
	int x_row, x_col;
	x_row = x_position / size; x_col = x_position % size;
	...
	if(x_row>0 && action !=2){
		...
		result = search_id_Astar(puzzle_next, cost_reach_node + 1, 1,cutoff);
		if(result == 1){
			return result;
		}
	}
	if(x_row<size-1 && action != 1){
		//move down
		...
		//puzzle_next.print_map();
		result = search_id_Astar(puzzle_next, cost_reach_node + 1, 2,cutoff);
		if(result == 1){
			return result;
		}
	}
	if(x_col>0 && action != 4){
		...
		//puzzle_next.print_map();
		result = search_id_Astar(puzzle_next, cost_reach_node + 1, 3,cutoff);
		if(result == 1){
			return result;
		}
	}
	if(x_col<size-1 && action != 3){
		//move right
		...
		//puzzle_next.print_map();
		result = search_id_Astar(puzzle_next, cost_reach_node + 1, 4,cutoff);
		if(result == 1){
			return result;
		}
	}
